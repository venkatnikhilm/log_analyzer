              ...
              {logs.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={7} className="text-center text-muted-foreground py-8">
                    No logs available. Upload a file to see log entries.
                  </TableCell>
                </TableRow>
              ) : (
                logs.map((log) => (
                  <TableRow key={log.id} className={cn("transition-colors", getRowColor(log.status, log.uri))}>
                    ...
                    <TableCell>{getStatusBadge(log.status)}</TableCell>
                    <TableCell className="text-sm">{log.bytes?.toLocaleString() || "-"}</TableCell>
                    <TableCell className="max-w-xs truncate text-xs text-muted-foreground">{log.user_agent || "-"}</TableCell>
                  </TableRow>
                ))
              )}
            </TableBody>
